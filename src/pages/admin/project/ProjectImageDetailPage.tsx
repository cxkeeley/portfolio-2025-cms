...
import AdminProjectImageLanguagesAPI, {
  PayloadAdminProjectImageLanguageCreate,
  PayloadAdminProjectImageLanguageUpdate,
...
import ProjectImageDetailCard from '@modules/project/components/ProjectImageDetailCard'
import ProjectImageFormModal, {
  ProjectImageFormModalShape,
} from '@modules/project/components/ProjectImageFormModal'
import ProjectImageLanguageDetailCard from '@modules/project/components/ProjectImageLanguageDetailCard'
import ProjectImageLanguageFormModal, {
  ProjectImageLanguageFormModalShape,
} from '@modules/project/components/ProjectImageLanguageFormModal'
...
const ProjectImageDetailPage: React.FC = () => {
  const intl = useIntl()
  const alert = useAlert()
  const toast = useToast()
  const queryClient = useQueryClient()
  const { projectId, projectImageId } = useParams()
  ...
  const { data, isLoading, isFetching, isError, refetch } = useQuery({
    queryKey: [QUERIES.ADMIN_PROJECT_IMAGE_DETAIL, { projectId, projectImageId }],
    queryFn: () => AdminProjectImagesAPI.get(projectImageId!),
    select: (response) => response.data.data.project_image,
  })
  ...
  const handleSubmitEditFormModal = async (values: ProjectImageFormModalShape) => {
    await AdminProjectImagesAPI.updateImage(projectImageId!, {
      image_file_path: values.image_file_path ?? '',
    })

    // update featured image on project detail
    queryClient.invalidateQueries([QUERIES.ADMIN_PROJECT_DETAIL, { projectId }])

    refetch()
    hideEditModal()
  }

  const handleSubmitCreateLanguageFormModal = async (values: ProjectImageLanguageFormModalShape) => {
    const payload = FormUtil.parseValues<PayloadAdminProjectImageLanguageCreate>(values)
    await AdminProjectImageLanguagesAPI.create({
      ...payload,
  ...
  const handleSubmitEditLanguageFormModal = async (
    ...
    language: ProjectImageLanguageModel
  ) => {
    const payload = FormUtil.parseValues<PayloadAdminProjectImageLanguageUpdate>(values)
    await AdminProjectImageLanguagesAPI.update(language.id, payload)
  ...
  const deleteLanguage = React.useCallback(
    async (language: ProjectImageLanguageModel, index: number) => {
      try {
        ...
        if (isConfirmed) {
          await AdminProjectImageLanguagesAPI.delete(language.id)
          refetch()
    ...
    return (
      <React.Fragment>
          ...
          >
            {hasLanguages ? (
              <React.Fragment>
                ...
                {activeLanguage && (
                  <React.Fragment>
                    <ProjectImageLanguageDetailCard
                      imageCaption={activeLanguage.image_caption}
                      imageAlt={activeLanguage.image_alt}
                      updatedAt={activeLanguage.updated_at}
                      cardTitle={<FormattedMessage id="project_image.section.card_detail_title" />}
                      cardToolbar={
                        <PermissionsControl allow={PermissionEnum.ADMIN_PROJECT_IMAGE_LANGUAGE_UPDATE}>
                          <Button
                            theme="primary"
                            variant="light"
        ...
        <ProjectImageFormModal
          modalTitle={<FormattedMessage id="project_image.section.modal_edit_title" />}
        ...
        <ProjectImageLanguageFormModal
          projectImageId={data.id}
          modalTitle={<FormattedMessage id="project_image.section.modal_create_language_title" />}
        ...
        {activeLanguage && (
          <ProjectImageLanguageFormModal
            modalTitle={<FormattedMessage id="project_image.section.modal_edit_language_title" />}
