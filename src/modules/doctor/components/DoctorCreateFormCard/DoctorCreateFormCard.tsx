...
type Props = {
  onSubmit: (values: DoctorCreateFormShape) => Promise<void>
  onCancel: () => void
}

const DoctorCreateFormCard: FC<Props> = ({ onSubmit, onCancel }) => {
  const alert = useAlert()
  const intl = useIntl()
  const [error, setError] = useState<string>()

  const initial: DoctorCreateFormShape = {
    degree: null,
    image_file_path: '',
    is_active: true,
    job_title: '',
