...
type Props = {
  initialValues?: Partial<TeamUpdateFormShape>
  initialThubmnailSrc?: string
  initialImageSrc?: string
  onSubmit: (values: TeamUpdateFormShape) => Promise<void>
  onCancel: () => void
}

const TeamUpdateFormCard: FC<Props> = ({ initialValues, onSubmit, onCancel, initialImageSrc, initialThubmnailSrc }) => {
  const alert = useAlert()
  const intl = useIntl()
  const [error, setError] = useState<string>()

  const initial: TeamUpdateFormShape = {
    degree: '',
    slug: '',
    image_file_path: '',
    is_active: true,
...
export type { TeamUpdateFormShape }

export default TeamUpdateFormCard
