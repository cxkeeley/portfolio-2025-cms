...
type Props = {
  initialValues?: TeamLanguageFormShape
  isShow: boolean
  title: ReactNode
  teamId: ID
  onSubmit: (values: TeamLanguageFormShape) => Promise<void>
  onCancel: () => void
  onExited?: () => void
}

const TeamLanguageFormModal: FC<Props> = ({ initialValues, isShow, title, teamId, onSubmit, onCancel, onExited }) => {
  const alert = useAlert()
  const [error, setError] = useState<string>()

  const initial: TeamLanguageFormShape = {
    language_id: null,
    quote: null,
    quote_author: null,
    ...initialValues,
            ...
            <Modal.Body className="scroll-y mx-5 mx-xl-15 my-7">
              ...
              <Row className="gap-7">
                ...
                <Col xs={12}>
                  <FormControl>
                    <FormControl.Label>
                      <FormattedMessage id="team_language.label.quote" />
                    </FormControl.Label>
                    <TextareaField
                      rows={5}
                      name="quote"
                ...
                <Col xs={12}>
                  <FormControl>
                    <FormControl.Label>
                      <FormattedMessage id="team_language.label.quote_author" />
                    </FormControl.Label>
