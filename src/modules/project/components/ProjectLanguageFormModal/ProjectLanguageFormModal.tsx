...
import LanguageForProjectLanguageFormSelectField from '@modules/language/components/LanguageForProjectLanguageFormSelectField'
...
type ProjectLanguageFormModalShape = FormShape<{
  address: string
  description: string
  language_id: Option
  name: string
...
type Props = {
  ...
  projectId: ID
  initialValues?: Partial<ProjectLanguageFormModalShape>
  disabledFields?: Partial<FormDisabledFlags<ProjectLanguageFormModalShape>>
  hiddenFields?: Partial<FormHiddenFlags<ProjectLanguageFormModalShape>>
  onSubmit: (values: ProjectLanguageFormModalShape) => Promise<void>
  onCancel: () => void
  onHide: () => void
}

const defaultValues: ProjectLanguageFormModalShape = {}

const ProjectLanguageFormModal: FC<Props> = (props) => {
  ...
  const handleSubmit = async (
    values: ProjectLanguageFormModalShape,
    helpers: FormikHelpers<ProjectLanguageFormModalShape>
  ) => {
    try {
  ...
  return (
    ...
    >
      ...
      <Formik<ProjectLanguageFormModalShape>
              ...
              <Row className="gap-7">
                {!props.hiddenFields?.language_id && (
                  <Col xs={12}>
                    <FormControl>
                      ...
                      <PermissionsControl
                        renderError={true}
                        allow={[PermissionEnum.ADMIN_LANGUAGE_OPTION_FOR_PROJECT_LANGUAGE_FORM]}
                      >
                        <LanguageForProjectLanguageFormSelectField
                          name="language_id"
                ...
                {!props.hiddenFields?.short_name && (
                  <Col xs={12}>
                    <FormControl>
                      <FormControl.Label isRequired>
                        <FormattedMessage id="project.label.short_name" />
                      </FormControl.Label>
                      <TextInputField
                        name="short_name"
                ...
                {!props.hiddenFields?.name && (
                  <Col xs={12}>
                    <FormControl>
                      <FormControl.Label isRequired>
                        <FormattedMessage id="project.label.name" />
                      </FormControl.Label>
                      <TextInputField
                        name="name"
                ...
                {!props.hiddenFields?.description && (
                  <Col xs={12}>
                    <FormControl>
                      <FormControl.Label isRequired>
                        <FormattedMessage id="project.label.description" />
                      </FormControl.Label>
                      <TextareaField
                        name="description"
                        rows={5}
...
export type { ProjectLanguageFormModalShape }

export default ProjectLanguageFormModal
